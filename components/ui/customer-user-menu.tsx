...
import { useCustomerAuth, type CustomerUser } from "@/contexts/customer-auth-context";

interface CustomerUserMenuProps {
  customer: CustomerUser | null;
}

export function CustomerUserMenu({ customer }: CustomerUserMenuProps) {
  ...
  const displayId = customer?.msmeId ?? "—";
          ...
          <div className="border-b border-white/[0.06] px-5 py-3.5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted">{displayId}</p>
              </div>
            </div>
          </div>
