  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className="flex min-h-9 items-center gap-2 rounded-lg border border-line bg-slate-50 px-3 text-xs text-muted transition-all duration-150 hover:border-trust/50 hover:bg-white hover:text-ink md:w-56 lg:w-64"
      >
        <Search className="h-3.5 w-3.5 shrink-0" />
        <span className="hidden md:inline">Search applications, MSMEs...</span>
        <span className="ml-auto hidden rounded border border-line bg-white px-1.5 py-0.5 text-[10px] font-medium md:inline">⌘K</span>
      </button>

      {open && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/20 pt-[15vh] backdrop-blur-sm animate-fade-in" onClick={() => setOpen(false)}>
          <div
            className="w-full max-w-lg rounded-xl border border-line bg-panel shadow-soft animate-slide-up"
          ...
          >
            <div className="flex items-center gap-3 border-b border-line px-4 py-3">
              ...
              />
              <button type="button" onClick={() => setOpen(false)} className="rounded-md p-1 text-muted hover:bg-slate-100 hover:text-ink">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="max-h-80 overflow-y-auto p-2">
              ...
              {results.length > 0 && query.length >= 1 && (
                <div className="space-y-0.5">
                  {(["MSME", "Application", "Report"] as const).map((cat) => {
                    ...
                    return (
                      <div key={cat}>
                        <p className="px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted">{cat}</p>
                        {items.map((item) => (
                          <Link
                            key={item.id}
                            href={item.href}
                            onClick={() => setOpen(false)}
                            className="flex items-center justify-between rounded-md px-3 py-2.5 text-sm transition-colors hover:bg-slate-50"
                          >
                            ...
                            <span className={cn(
                              ...
                              cat === "MSME" ? "bg-trust-light text-trust" : cat === "Application" ? "bg-growth-light text-growth" : "bg-slate-100 text-muted"
                            )}>
                              {cat}
                            </span>
                          </Link>
              ...
              {query.length === 0 && (
                <p className="px-3 py-6 text-center text-sm text-muted">Type to search across MSMEs, applications, and reports</p>
              )}
            </div>
          </div>
        </div>
